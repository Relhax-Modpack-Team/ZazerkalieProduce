...
{
    class EpicRandomFullStatsSwfWorker : BaseSwfWorker
    {
        public EpicRandomFullStatsSwfWorker(Options options) : base(options)
        {
        }

        public override string SwfName { get; set; } = "epicRandomFullStats.swf";
        ...
        public override void DoUnmirror(ref XDocument xdoc)
        {
            ...
            if (options.Mode == Options.OutputMode.notext)
            {
                elements =
                xdoc.Descendants()
                    .Where(x => x.Attribute("name") != null && x.Attribute("name").Value.StartsWith("vehicleLevel"));
                foreach (var element in elements)
                {
